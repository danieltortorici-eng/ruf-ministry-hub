...
- A newly typed transient capture that is dismissed while Backend Quick Grab is pending cancels that capture-owned mutation, atomically removes its processing snapshot and Auto Memory residue, removes its capture Undo entry, ignores late success or rejection, and refuses to close when deletion persistence fails. Repeated backend failures reuse one capture-owned cancellation owner rather than accumulating hidden recovery blockers. While deletion is pending, the review is `aria-busy`, announces a polite live status, and disables every control; a failed write retains the capture and exposes only the two equivalent **Retry delete** actions. Proposal storage also rejects a Quick Grab proposal whose source capture no longer exists.
...
- Quick Grab person inference uses only a unique normalized full-name phrase or, when no full-name phrase matches, a unique whole first-name token. Substring, same-first-name, duplicate-full-name, explicit locked-person precedence, and pre-approval zero-record contracts are covered with fictional fixtures. Persisted stale links are exercised across local labels/privacy, Search, cleanup classification, linked counts, reviewed backend context, outbound candidate packets, every proposal type that carries a source Quick Grab, proposal validation, and converted-source reconciliation. A missing lock target fails closed before backend context, payload construction, or request; an explicit human choice may resolve it locally. Proposal-carried source references must all agree on exactly one current live Quick Grab, while conflicting or missing sources block before any record or source conversion. Adversarial top-level/result/action IDs and saved legacy proposals cannot override current source truth, repeated approval cannot duplicate the correctly linked record, and a 500-person/600-capture deterministic scale contract proves bulk Search reuses one people index instead of rebuilding it per capture.
