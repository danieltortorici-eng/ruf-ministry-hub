...
check(html.includes('<a class="skip-link" href="#main-content" hidden>Skip to main content</a>'), "the keyboard skip link starts outside the touch accessibility tree");
...
check(/\.skip-link\[hidden\]\s*\{[\s\S]*?display:\s*none/.test(html), "the initial hidden state removes the skip link from layout and accessibility geometry");
...
check(html.includes("function revealSkipLinkForKeyboard(event)") && html.includes('event.key !== "Tab"') && html.includes("event.altKey") && html.includes("event.ctrlKey") && html.includes("event.metaKey") && html.includes("event.shiftKey") && html.includes("document.activeElement !== document.body") && html.includes("document.activeElement !== document.documentElement") && html.includes("event.preventDefault();") && html.includes("skipLink.hidden = false;") && html.includes("skipLink.focus({ preventScroll: true });"), "only the first plain keyboard Tab from untouched page entry reveals and focuses the skip link");
check(html.includes('window.addEventListener("keydown", revealSkipLinkForKeyboard, true);') && html.includes('window.removeEventListener("keydown", revealSkipLinkForKeyboard, true);'), "keyboard skip-link activation is one-shot and capture-phase");
