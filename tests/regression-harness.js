...
function testServiceWorkerShape() {
  assert(serviceWorkerSource.includes("ruf-ministry-hub-v23-ai-confirm-actions"), "service worker cache version is bumped");
  assert(serviceWorkerSource.includes('"index.html"'), "service worker caches redirect entry point");
  assert(!serviceWorkerSource.includes("ruf-ministry-hub-icon.svg"), "service worker does not reference removed SVG icon");
...
function testRedirectAndServiceWorkerRouting() {
  ...
  assert(serviceWorkerSource.includes('url.pathname.startsWith("/api/")') && serviceWorkerSource.includes("if (url.pathname.startsWith(\"/api/\")) return;"), "service worker bypasses API routes");
  assert(serviceWorkerSource.includes('path === "/ruf-ministry-hub"') && serviceWorkerSource.includes("caches.match(HTML_ENTRY)"), "service worker serves extensionless app route as direct HTML");
  assert(serviceWorkerSource.includes('path === "/ruf-ministry-hub.html"'), "service worker handles direct HTML route");
}
