...
Do not store OpenAI keys in the repo or local env files. Set runtime secrets only in Cloudflare:

- `OPENAI_API_KEY`
- `OPENAI_MODEL`
- `AI_MOCK_MODE`

If `OPENAI_API_KEY` is missing, or if `AI_MOCK_MODE=true`, the Quick Grab AI endpoint returns a mock proposal safely. AI proposals never write records directly; Daniel must approve before saving.
...
- No OpenAI API key appears in frontend files, test files, env files, git history, or committed code.
