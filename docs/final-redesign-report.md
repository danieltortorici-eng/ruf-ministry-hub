...
| `node audit-secrets.mjs` | Exit 0 across 86 tracked/proposed files and six browser assets; ignored secret files were not read. |
...
Confirmed defects were fixed and covered by regression tests: linked sensitive Quick Grab Search and Today task/prayer/capture exposure; profile/Brief Me/Follow Up leakage from tasks linked to sensitive prayers; backup maintenance competing with meaningful ministry work; backend transmission of raw text linked to `Do Not Send to AI` people; mixed legacy privacy flags; unmasked sensitive final-approval duplicate warnings; proposal privacy tier loss after source deletion; gate-marked proposal redaction; Today archived/malformed-snooze leaks; service-worker required-shell, cache-write, and non-OK navigation failures; and large-text Right Now layout. Canonical deploy syntax and asset checks were expanded to cover the deploy tree directly.

Independent evidence includes `npm test`, `node tests/regression-harness.js`, UTC/Pacific-Kiritimati/America-Adak/America-Los_Angeles timezone runs, and `node tests/deployment-config-regression.js` all exiting 0 after the final CSP hash synchronization and `APP_VERSION=2026.07.15-calm-os-qa-v4`/`ruf-ministry-hub-v47-calm-os-qa-v4` cache bump, plus focused recovery/accessibility/service-worker/AI/deployment checks, generated-output and secret audits, and DevTools verification at 390px/430px with offline reload, Lighthouse accessibility 100, and LCP approximately 582ms.
