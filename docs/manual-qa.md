...
- Confirm unknown `/api/*` and nested `/api/ai/*` paths return metadata-only JSON `404`, not the app HTML; `/api/ai/health` and `/api/ai/quick-grab` must remain the only active release API routes.
