...
### PD-016 — Gate rendering on recovery and centralize accessible interaction state

- **Decision:** Do not render or seed ministry content until local and IndexedDB recovery resolves; fail closed for corrupt/ambiguous/encrypted state. Serialize encrypted persistence by revision and recover drafts/proposal edits without recovering final approval. At the UI boundary, use one modal-focus implementation, one invalid-field reporter, explicit labels/groups, route-heading focus, and one shared target/contrast contract.
- **Alternatives considered:** Render demo data while IndexedDB loads; keep best-effort encrypted timers; recover the final approval checkbox; patch focus and labels independently in each sheet; rely on toast text and color alone.
- **Reason:** Early rendering can overwrite real recovery data, concurrent encrypted saves can commit stale snapshots, and recovering final approval would weaken human consent. Shared accessibility primitives prevent sheet-by-sheet drift and support interrupted, one-handed use.
- **Cognitive-load effect:** Startup has one clear wait/retry state; interrupted work returns to the exact unfinished decision without pretending approval; sheets and validation behave consistently; navigation gives predictable focus context.
