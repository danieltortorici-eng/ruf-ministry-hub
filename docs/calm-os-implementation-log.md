...
- Added fail-closed precedence for mixed legacy privacy flags and persisted a `Do Not Send to AI` tier plus redacted previews when a review gate marks a source after proposal creation.
- Confirmed and fixed a recommendation-order defect where a due backup reminder could appear alongside meaningful ministry work; maintenance now appears only when no tier-1–8 ministry recommendation exists, with regression coverage for both sides of the gate.
- Extended linked-record privacy transitively through sensitive prayer requests: profile history, Right Now, Brief Me, Follow Up, and AI context now mask or block otherwise-normal tasks linked to a sensitive prayer.
...
- Latest focused evidence: `node tests/regression-harness.js` exit 0; `node tests/deployment-config-regression.js` exit 0 after synchronizing the current CSP hash; syntax, generated-output, asset, secret, accessibility, recovery, service-worker, and full `npm test` checks pass. The final deploy marker is `APP_VERSION=2026.07.15-calm-os-qa-v4` with service-worker cache `ruf-ministry-hub-v47-calm-os-qa-v4`. UTC, Pacific/Kiritimati, America/Adak, and America/Los_Angeles timezone runs also exit 0 after the final documentation checkpoint.
